...
if ! test -d ./main; then
  echo "The main directory not exist => mkdir main"
  mkdir main
fi

if ! test -d ./main/main.tex; then
  echo "The main/main.tex file not exist => touch main/main.tex"
  touch main/main.tex
fi


echo "----- md2tex MARKDOWN -----"
for markdown in "."/*.md
do
	filename="${markdown%.*}"
  filename="${filename#./}"
	echo "== $filename =="
	md2tex "$filename.md" -o "main/$filename.tex" -t
	echo "---------------------------"
done


# Write the latex template to the main.tex
cat > main/main.tex << EOF
...
do
	filename="${markdown%.*}"
  filename="${filename#./}"
	echo "\fancyfoot[R]{\texttt{$filename}}" >> main/main.tex

  echo "\subfile{$filename.tex}" >> main/main.tex
done

echo "% ================================================================================" >> main/main.tex
echo "\end{document}" >> main/main.tex


echo "==========================="
echo "output file: main/main.tex"
echo "==========================="
